...
namespace InternalPayments.Classes
{
    ...
    class Global
    {
        ...
        public static string payeRatesSQL="";
...
, a.pay_trns_type, a.crncy_id, a.pymnt_desc " +
        ...
        public static long get_MsPyInvoiceID(long mspyID)
        {
            string strSql = "SELECT sales_invoice_id " +
             "FROM pay.pay_itm_trnsctns a " +
             "WHERE(a.mass_pay_id = " + mspyID +
             " and a.sales_invoice_id>0 and a.pymnt_vldty_status='VALID' and a.src_py_trns_id<=0) ORDER BY mass_pay_id DESC LIMIT 1 OFFSET 0";
      ...
      ELSE z.mass_pay_id END from pay.pay_itm_trnsctns z where z.pay_trns_id=v.source_trns_id) IS NULL))";
        ...
        public static void updateItm(int orgid, long itmid, string itnm, string itmDesc,
         ...
         bool useSQL, bool isenbld, int costAcnt, int balsAcnt,
          ...
          int itmMnID, bool isRetro, int retroID, int invItmID, bool allwEdit, bool createsAcctng, string effctOnOrgDbt)
        {
            Global.mnFrm.cmCde.Extra_Adt_Trl_Info = "";
            string dateStr = Global.mnFrm.cmCde.getDB_Date_time();
         ...
         ", is_retro_element='" + Global.mnFrm.cmCde.cnvrtBoolToBitStr(isRetro) +
              ...
              "', effct_on_org_debt='" + effctOnOrgDbt.Replace("'", "''") + "' WHERE item_id=" + itmid;
            Global.mnFrm.cmCde.updateDataNoParams(updtSQL);
        }
        ...
        public static void createItm(int orgid, string itnm, string itmDesc,
         ...
         bool useSQL, bool isenbld, int costAcnt, int balsAcnt,
          ...
          bool isRetro, long retroID, long invItmID, bool allwEdit, bool createsAcctng, string effctOnOrgDbt)
        {
            string dateStr = Global.mnFrm.cmCde.getDB_Date_time();
...
a.row_id, b.item_maj_type, b.pay_run_priority, b.item_code_name " +
        ...
        public static DataSet get_PayeRates()
        {
            string selSQL = "SELECT rates_id, level_order, rates_amount, tax_percent " +
                  "FROM pay.pay_paye_rates ORDER BY level_order ASC, rates_id ASC";
            DataSet dtst = Global.mnFrm.cmCde.selectDataNoParams(selSQL);
            Global.payeRatesSQL = selSQL;
            return dtst;
        }

        public static void createPayeRates(int lvlNo, double rateAmount, double txRate, string dateStr)
        {
            Global.mnFrm.cmCde.Extra_Adt_Trl_Info = "";
            string insSQL = @"INSERT INTO pay.pay_paye_rates(
            rates_amount, tax_percent, level_order, created_by, 
            creation_date, last_update_by, last_update_date)
    VALUES (" + rateAmount + ", " + txRate + ", " + lvlNo +
            ", " + Global.myPay.user_id + ", '" + dateStr + "', " + Global.myPay.user_id + ",  '" + dateStr + "')";
            Global.mnFrm.cmCde.insertDataNoParams(insSQL);
        }

        public static void updatePayeRates(long rateID, int lvlNo, double rateAmount, double txRate, string dateStr)
        {
            Global.mnFrm.cmCde.Extra_Adt_Trl_Info = "";
            string updtSQL = @"UPDATE pay.pay_paye_rates SET 
            rates_amount=" + rateAmount + ", tax_percent=" + txRate + ", level_order=" + lvlNo + 
            ", last_update_by=" + Global.myPay.user_id + ", last_update_date='" + dateStr + "' WHERE rates_id=" + rateID;
            Global.mnFrm.cmCde.updateDataNoParams(updtSQL);
        }
        ...
        public static void createRqrdItems()
        {
            ...
            string[] effctOnOrgDbt = { "Decrease", "Decrease", "Increase" };
            string[] costAccntNo = { "", "", "" };
            ...
            for (int i = 0; i < itmNm.Length; i++)
            {
                ...
                if (itm_id_in <= 0 && Global.mnFrm.cmCde.Org_id > 0)
                {
                    Global.createItm(Global.mnFrm.cmCde.Org_id, itmNm[i], itmDesc[i], itmMajTyp[i], itmMinTyp[i], itmUOM[i],
                      ...
                      isRetroElmnt, retrItmID, invItmID, allwEditing, creatsActng, effctOnOrgDbt[i]);
