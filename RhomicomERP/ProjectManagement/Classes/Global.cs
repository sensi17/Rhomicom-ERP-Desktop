...
namespace ProjectManagement.Classes
{
  ...
  class Global
  {
    ...
    public static ProjectManagement myProj = new ProjectManagement();
    public static mainForm mnFrm = null;
    public static projectRecsForm projFrm = null;
    public static accntSetupForm actStpFrm = null;
    public static resourcesForm resourcesFrm = null;
    public static srchProjectForm srchProjFrm = null;
    ...
    public static string[] dfltPrvldgs = {"View Project Management", 
   ...
   string filNm)
    {
      ...
      string updtSQL = "UPDATE attn.attn_doc_attchmnts SET " +
            "attnd_rec_id=" + recID +
                        ...
                        "', last_update_by=" + Global.myProj.user_id +
...
Comment
  ...
  FROM attn.attn_attendance_events_rslts a, attn.attn_attendance_events_mtrcs b " +
      ...
      DataSet dtst = Global.mnFrm.cmCde.selectDataNoParams(strSql);
      Global.resourcesFrm.rec_det_SQL = strSql;
      return dtst;
    }
 ...
 bool isEnbld, int itmID, int evntID)
    {
      ...
      string insSQL = @"INSERT INTO attn.event_price_categories(
            ...
            "', " + itmID + ", " + Global.myProj.user_id + ", '" + dateStr +
            "', " + Global.myProj.user_id + ", '" + dateStr +
            "', '" +
            Global.mnFrm.cmCde.cnvrtBoolToBitStr(isEnbld) + "')";
      Global.mnFrm.cmCde.insertDataNoParams(insSQL);
...
CASE WHEN a.event_typ='R' THEN ' R:RECURRING' ELSE 'NR:NON-RECURRING' END, a.host_prsn_id, 
    ...
    public static DataSet get_One_EvntMtrcs(int evntid)
    {
      ...
      DataSet dtst = Global.mnFrm.cmCde.selectDataNoParams(strSql);
      Global.resourcesFrm.recDt_SQL = strSql;
      return dtst;
    }
    ...
    public static DataSet get_One_EvntPrices(int evntid)
    {
      ...
      DataSet dtst = Global.mnFrm.cmCde.selectDataNoParams(strSql);
      Global.resourcesFrm.recPrc_SQL = strSql;
      return dtst;
    }
...
" OFFSET " + (Math.Abs(offset * limit_size)).ToString();
      ...
      Global.resourcesFrm.rec_SQL = strSql;
      DataSet dtst = Global.mnFrm.cmCde.selectDataNoParams(strSql);
      return dtst;
    }
...
string tmeMajDivEnd, string tmeMinDivEnd)
    {
      ...
      string insSQL = @"INSERT INTO attn.attn_time_table_details(
            ...
            "', '" + tmeMinDiv.Replace("'", "''") +
            "', " + hostID + ", " + Global.myProj.user_id + ", '" + dateStr +
            "', " + Global.myProj.user_id + ", '" + dateStr +
            "', " + vnuID + ", '" +
...
string tmeMajDivEnd, string tmeMinDivEnd)
    {
      ...
      string insSQL = @"UPDATE attn.attn_time_table_details SET 
            ...
            "', assgnd_host_id=" + hostID + ", last_update_by=" + Global.myProj.user_id +
            ", last_update_date='" + dateStr +
...
" OFFSET " + (Math.Abs(offset * limit_size)).ToString();
      ...
      Global.actStpFrm.rec_SQL = strSql;
      DataSet dtst = Global.mnFrm.cmCde.selectDataNoParams(strSql);
      return dtst;
    }
...
"WHERE ((a.org_id = " + orgID + ")" + whrcls + ")";
    ...
    public static DataSet get_One_TmeTbl_DetLns(string searchWord, string searchIn, long offset, int limit_size, long tmtblID)
    {
      ...
      DataSet dtst = Global.mnFrm.cmCde.selectDataNoParams(strSql);
      Global.actStpFrm.rec_det_SQL = strSql;
      return dtst;
    }
...
scm.get_doc_smry_typ_amnt(invc_hdr_id, invc_type,'5Grand Total'), evnt_cost_category, COALESCE(a.no_of_adults,1)
  FROM scm.scm_sales_invc_hdr y LEFT OUTER JOIN hotl.checkins_hdr a ON ((a.check_in_id = y.other_mdls_doc_id or (a.prnt_chck_in_id=y.other_mdls_doc_id and y.other_mdls_doc_id>0))
and (a.doc_type=y.other_mdls_doc_type or (a.prnt_doc_typ=y.other_mdls_doc_type and a.prnt_doc_typ != '')))
  WHERE event_rgstr_id=" + rgstrID;
...
Category
      ...
      DataSet dtst = Global.mnFrm.cmCde.selectDataNoParams(strSql);
      Global.projFrm.rec_Cost_SQL = strSql;
      return dtst;
    }
...
string dtetmein, string dtetmeout, bool isPresent, string attnCmmnts, string name_desc,
      int noAdlts, long cstmrID, string vstrClsf)
    {
      //visitor_name_desc, no_of_adults, no_of_chldrn, customer_id, visitor_classfctn
      if (dtetmein != "")
      {
  ...
  System.Globalization.CultureInfo.InvariantCulture).ToString("yyyy-MM-dd HH:mm:ss");
      ...
      string insSQL = @"INSERT INTO attn.attn_attendance_recs(
            ...
            created_by, creation_date, last_update_by, last_update_date, 
            is_present, attn_comments,visitor_name_desc, no_of_persons, customer_id, visitor_classfctn) " +
...
string dtetmein, string dtetmeout, bool isPresent, string attnCmmnts, string name_desc,
  ...
  string sitedesc, string cntctPrsn, string cntctNos, string email,
    ...
    public static bool isAttnRgstrLnInUse(long tmetblLnid)
    {
      return false;
    }
 ...
 WHERE d.event_id = e.event_id and d.time_table_det_id = " + tmtblDetID + "";
    ...
    public static DataSet get_One_ProjsDet(long proHdrID)
    {
      string strSql = @"SELECT proj_header_id, proj_name, proj_desc, proj_type, indstrl_sector, 
       proj_size, proj_site_location, 
to_char(to_timestamp(proj_start_date,'YYYY-MM-DD HH24:MI:SS'),'DD-Mon-YYYY HH24:MI:SS'), 
to_char(to_timestamp(proj_end_date,'YYYY-MM-DD HH24:MI:SS'),'DD-Mon-YYYY HH24:MI:SS'), 
       proj_mngr_prsn_id, proj_cstmr_id, proj_cstmr_site_id, created_by, 
       creation_date, last_update_by, last_update_date, org_id
  FROM proj.project_records a 
  ...
  FROM attn.attn_attendance_recs_hdr a " +
    ...
    public static DataSet get_Basic_ProjRecs(string searchWord, string searchIn,
 Int64 offset, int limit_size, int orgID, bool showSelf)
    {
      string strSql = "";
      string whrcls = "";
      string extrWhr = "";
      ...
      if (showSelf)
      {
        long prsnID = Global.mnFrm.cmCde.getUserPrsnID(Global.mnFrm.cmCde.User_id);
        extrWhr = " and (a.created_by = " + Global.mnFrm.cmCde.User_id +
          " or a.proj_mngr_prsn_id = " + prsnID + ")";
      }
      if (searchIn == "Project Name")
      {
        whrcls = " AND (a.proj_name ilike '" + searchWord.Replace("'", "''") +
       "')";
      }
      else if (searchIn == "Project Description")
      {
        whrcls = " AND (a.proj_desc ilike '" + searchWord.Replace("'", "''") +
       "')";
      }
      else if (searchIn == "Project Type")
      {
        whrcls = " AND (a.proj_type ilike '" + searchWord.Replace("'", "''") +
       "')";
      }
      else if (searchIn == "Sector")
      {
        whrcls = " AND (a.indstrl_sector ilike '" + searchWord.Replace("'", "''") +
       "')";
      }
      else if (searchIn == "Size")
      {
        whrcls = " AND (a.proj_size ilike '" + searchWord.Replace("'", "''") +
       "')";
      }
      else if (searchIn == "Project Number")
      {
        whrcls = " AND (trim(to_char(a.proj_header_id, '999999999999999999999999999')) ilike '" + searchWord.Replace("'", "''") +
       "')";
      }
      else if (searchIn == "Project Location")
      {
        whrcls = " AND (proj_site_location ilike '" + searchWord.Replace("'", "''") +
       "')";
      }
      else if (searchIn == "Project Dates")
      {
        whrcls = " AND (to_char(to_timestamp(a.proj_start_date,'YYYY-MM-DD HH24:MI:SS'),'DD-Mon-YYYY HH24:MI:SS') ilike '" + searchWord.Replace("'", "''") +
       "' or to_char(to_timestamp(a.proj_end_date,'YYYY-MM-DD HH24:MI:SS'),'DD-Mon-YYYY HH24:MI:SS') ilike '" + searchWord.Replace("'", "''") +
       "')";
      }

      strSql = "SELECT a.proj_header_id, a.proj_name " +
"FROM proj.project_records a " +
"WHERE ((a.org_id = " + orgID + ")" + whrcls + extrWhr +
") ORDER BY a.proj_header_id DESC LIMIT " + limit_size +
" OFFSET " + (Math.Abs(offset * limit_size)).ToString();

      Global.projFrm.rec_SQL = strSql;
      DataSet dtst = Global.mnFrm.cmCde.selectDataNoParams(strSql);
      return dtst;
    }

    public static long get_Total_ProjRecs(string searchWord, string searchIn, int orgID, bool showSelf)
    {
      string strSql = "";
      string whrcls = "";
      string extrWhr = "";
...
"WHERE ((a.org_id = " + orgID + ")" + whrcls + extrWhr + ")";
    ...
    public static DataSet get_One_ProjRec_DetLns(string searchWord, string searchIn, long offset,
      int limit_size, long projHdrID)
    {
      string strSql = "";
      string whrcls = "";
      /*
       Comments/Remarks
        ...
        Team Member Name/ID
       */
      if (searchIn == "Team Member Name/ID")
      {
        whrcls = @" AND ((CASE WHEN a.customer_id <= 0 and a.person_id <= 0 THEN a.member_name_desc 
           WHEN a.person_id>0 THEN prs.get_prsn_surname(a.person_id) || ' (' || prs.get_prsn_loc_id(a.person_id) || ')' 
            ELSE scm.get_cstmr_splr_name(a.customer_id)||scm.get_cstmr_splr_name(a.lnkd_firm_id) END) ilike '" + searchWord.Replace("'", "''") +
       "')";
      }
      else if (searchIn == "From Date/Time")
      {
        whrcls = " AND (to_char(to_timestamp(a.date_time_in,'YYYY-MM-DD HH24:MI:SS'),'DD-Mon-YYYY HH24:MI:SS') ilike '" + searchWord.Replace("'", "''") +
       "')";
      }
      else if (searchIn == "To Date/Time")
      {
      ...
      {
        whrcls = " AND (a.proj_comments ilike '" + searchWord.Replace("'", "''") +
       "')";
      }
      strSql = @"SELECT projs_rec_id, projs_hdr_id, person_id, prs.get_prsn_loc_id(a.person_id), 
      CASE WHEN a.customer_id <= 0 and a.person_id <= 0 THEN a.member_name_desc  
           WHEN a.person_id>0 THEN prs.get_prsn_surname(a.person_id) || ' (' || prs.get_prsn_loc_id(a.person_id) || ')' 
      ...
      a.proj_comments, a.team_member_classfctn, a.customer_id, a.lnkd_firm_id, scm.get_cstmr_splr_name(a.lnkd_firm_id)
  FROM proj.proj_team_members a " +
        "WHERE((a.projs_hdr_id = " + projHdrID + ")" + whrcls + ") ORDER BY 5,4,1 LIMIT " + limit_size +
        " OFFSET " + (Math.Abs(offset * limit_size)).ToString();

      DataSet dtst = Global.mnFrm.cmCde.selectDataNoParams(strSql);
      Global.projFrm.rec_det_SQL = strSql;
      return dtst;
    }

    public static long get_Total_ProjRecs_DetLns(string searchWord, string searchIn, long projHdrID)
    {
      string strSql = "";
      string whrcls = "";
      /*
      ...
      strSql = @"SELECT count(1)
  FROM proj.proj_team_members a " +
        "WHERE((a.projs_hdr_id = " + projHdrID + ")" + whrcls + ")";
    ...
    public static DataSet get_One_ProjTaskLns(string searchWord, string searchIn, long offset,
      int limit_size, long projHdrID)
    {
      string strSql = "";
      string whrcls = "";
       ...
       */
      if (searchIn == "From Date/Time")
      {
        whrcls = " AND (to_char(to_timestamp(a.estimated_start_date,'YYYY-MM-DD HH24:MI:SS'),'DD-Mon-YYYY HH24:MI:SS') ilike '" + searchWord.Replace("'", "''") +
       "')";
      }
      else if (searchIn == "To Date/Time")
      {
        whrcls = " AND (to_char(to_timestamp(a.estimated_end_date,'YYYY-MM-DD HH24:MI:SS'),'DD-Mon-YYYY HH24:MI:SS') ilike '" + searchWord.Replace("'", "''") +
       "')";
      }
      else if (searchIn == "Team Member Responsible")
      {
        whrcls = @" AND (proj.get_team_member_name(a.tm_member_rspsbl_id) ilike '" + searchWord.Replace("'", "''") +
       "')";
      }
      else if (searchIn == "Task Classification")
      {
        whrcls = " AND (a.proj_task_clsfctn ilike '" + searchWord.Replace("'", "''") +
       "')";
      }
      else if (searchIn == "Task Name/Description")
      {
        whrcls = " AND (a.task_name_desc ilike '" + searchWord.Replace("'", "''") +
       "')";
      }
      else if (searchIn == "Comments/Remarks")
      {
      ...
      strSql = @"SELECT proj_task_asgnmnt_id, projs_hdr_id, tm_member_rspsbl_id, proj.get_team_member_name(a.tm_member_rspsbl_id), 
      task_name_desc, 
      CASE WHEN a.estimated_start_date != '' THEN to_char(to_timestamp(a.estimated_start_date,'YYYY-MM-DD HH24:MI:SS'),'DD-Mon-YYYY HH24:MI:SS') ELSE '' END, 
      CASE WHEN a.estimated_end_date != '' THEN to_char(to_timestamp(a.estimated_end_date,'YYYY-MM-DD HH24:MI:SS'),'DD-Mon-YYYY HH24:MI:SS') ELSE '' END,
           CASE WHEN a.actual_start_date != '' THEN to_char(to_timestamp(a.actual_start_date,'YYYY-MM-DD HH24:MI:SS'),'DD-Mon-YYYY HH24:MI:SS') ELSE '' END, 
      CASE WHEN a.actual_end_date != '' THEN to_char(to_timestamp(a.actual_end_date,'YYYY-MM-DD HH24:MI:SS'),'DD-Mon-YYYY HH24:MI:SS') ELSE '' END,
 a.comments_remarks, a.proj_task_clsfctn, a.is_completed  
  FROM proj.project_task_assgnmnts a " +
        "WHERE((a.projs_hdr_id = " + projHdrID + ")" + whrcls + ") ORDER BY 5,4,1 LIMIT " + limit_size +
        " OFFSET " + (Math.Abs(offset * limit_size)).ToString();

      DataSet dtst = Global.mnFrm.cmCde.selectDataNoParams(strSql);
      Global.projFrm.rec_tasks_SQL = strSql;
      return dtst;
    }

    public static long get_Total_ProjTaskLns(string searchWord, string searchIn, long projHdrID)
    {
      string strSql = "";
      string whrcls = "";
      /*
  ...
  FROM proj.project_task_assgnmnts a " +
    ...
    public static DataSet get_One_ProjRsrcLns(string searchWord, string searchIn, long offset,
      int limit_size, long projHdrID)
    {
      string strSql = "";
      string whrcls = "";
      ...
      else if (searchIn == "Resource Type")
      {
        whrcls = " AND (a.rsrc_type ilike '" + searchWord.Replace("'", "''") +
       "')";
      }
      else if (searchIn == "Resource Name/Description")
      {
        whrcls = " AND (a.rsrc_name_desc ilike '" + searchWord.Replace("'", "''") +
       "')";
      }
      else if (searchIn == "Comments/Remarks")
      {
      ...
      strSql = @"SELECT proj_rsrc_alloc_id, projs_hdr_id, proj_rsrc_setup_id, rsrc_type, rsrc_name_desc, 
      CASE WHEN a.start_date != '' THEN to_char(to_timestamp(a.start_date,'YYYY-MM-DD HH24:MI:SS'),'DD-Mon-YYYY HH24:MI:SS') ELSE '' END, 
      CASE WHEN a.end_date != '' THEN to_char(to_timestamp(a.end_date,'YYYY-MM-DD HH24:MI:SS'),'DD-Mon-YYYY HH24:MI:SS') ELSE '' END,
 a.comments_remarks, a.supplier_id, scm.get_cstmr_splr_name(a.supplier_id)   
  FROM proj.project_rsrc_alloctns a " +
        "WHERE((a.projs_hdr_id = " + projHdrID + ")" + whrcls + ") ORDER BY 5,4,1 LIMIT " + limit_size +
        " OFFSET " + (Math.Abs(offset * limit_size)).ToString();

      DataSet dtst = Global.mnFrm.cmCde.selectDataNoParams(strSql);
      Global.projFrm.rec_rsrc_SQL = strSql;
      return dtst;
    }

    public static long get_Total_ProjRsrcLns(string searchWord, string searchIn, long projHdrID)
    {
      string strSql = "";
      string whrcls = "";
      /*
  ...
  FROM proj.project_rsrc_alloctns a " +
    ...
    public static long getNewProjsHdrID()
    {
      //string strSql = "select nextval('accb.accb_trnsctn_batches_batch_id_seq'::regclass);";
      string strSql = "select  last_value from proj.project_records_proj_header_id_seq";
      DataSet dtst = Global.mnFrm.cmCde.selectDataNoParams(strSql);
      if (dtst.Tables[0].Rows.Count > 0)
      {
    ...
    public static long doesProjHvPrsn(long prsnid, long projsHdrID)
    {
      string selSQL = "SELECT proj_team_member_id " +
                  "FROM proj.proj_team_members WHERE ((person_id = " + prsnid +
                  ") and (projs_hdr_id = " + projsHdrID + "))";
      DataSet dtst = Global.mnFrm.cmCde.selectDataNoParams(selSQL);
      if (dtst.Tables[0].Rows.Count > 0)
      {
        return long.Parse(dtst.Tables[0].Rows[0][0].ToString());
    ...
    public static long doesProjTeamHvCstmr(long cstmrID, long projsHdrID)
    {
      string selSQL = "SELECT proj_team_member_id " +
                  "FROM proj.proj_team_members WHERE ((customer_id = " + cstmrID +
                  ") and (projs_hdr_id = " + projsHdrID + "))";
      DataSet dtst = Global.mnFrm.cmCde.selectDataNoParams(selSQL);
      if (dtst.Tables[0].Rows.Count > 0)
      {
    ...
    public static DataSet get_Proj_SrchLns(
      string searchWord, string searchIn, long offset,
   int limit_size, int orgid, string dte1, string dte2, bool showSelf)
    {
...
System.Globalization.CultureInfo.InvariantCulture).ToString("yyyy-MM-dd HH:mm:ss");
      ...
      strSql = @"SELECT a.proj_header_id, a.proj_name, a.proj_desc, a.proj_type, a.indstrl_sector, 
       a.proj_size, a.proj_site_location, a.proj_start_date, a.proj_end_date, 
       a.proj_mngr_prsn_id, a.proj_cstmr_id, a.proj_cstmr_site_id " +
"FROM proj.project_records a " +
"WHERE ((a.org_id = " + orgid + ") and ((to_timestamp(a.proj_start_date,'YYYY-MM-DD HH24:MI:SS') between to_timestamp('" + dte1 +
       "','YYYY-MM-DD HH24:MI:SS') AND to_timestamp('" + dte2 + "','YYYY-MM-DD HH24:MI:SS')) or (to_timestamp(a.proj_end_date,'YYYY-MM-DD HH24:MI:SS') between to_timestamp('" + dte1 +
       "','YYYY-MM-DD HH24:MI:SS') AND to_timestamp('" + dte2 + "','YYYY-MM-DD HH24:MI:SS')))" + whrcls + extrWhr +
") ORDER BY a.proj_header_id DESC LIMIT " + limit_size +
" OFFSET " + (Math.Abs(offset * limit_size)).ToString();

      DataSet dtst = Global.mnFrm.cmCde.selectDataNoParams(strSql);
      Global.srchProjFrm.rec_SQL = strSql;
      return dtst;
    }

    public static long get_Total_Proj_SrchLns(
      string searchWord, string searchIn, int orgid, 
      string dte1, string dte2, bool showSelf)
    {
      dte1 = DateTime.ParseExact(
dte1, "dd-MMM-yyyy HH:mm:ss",
System.Globalization.CultureInfo.InvariantCulture).ToString("yyyy-MM-dd HH:mm:ss");
...
y.invc_type, a.prnt_chck_in_id,a.prnt_doc_typ, COALESCE(y.enbl_auto_misc_chrges,'0') " +
   @"FROM hotl.checkins_hdr a 
LEFT OUTER JOIN hotl.service_types d ON (a.service_type_id=d.service_type_id )
LEFT OUTER JOIN hotl.rooms b ON (a.service_det_id = b.room_id)
LEFT OUTER JOIN scm.scm_sales_invc_hdr y ON ((a.check_in_id = y.other_mdls_doc_id or (a.prnt_chck_in_id=y.other_mdls_doc_id and y.other_mdls_doc_id>0))
...
to_char(to_timestamp(a.creation_date,'YYYY-MM-DD HH24:MI:SS'),'DD-Mon-YYYY HH24:MI:SS'), 
  ...
  string docType, string strtDte, string endDte, int srvsTypID,
   ...
   SET doc_num='" + docNum.Replace("'", "''") +
            ...
            "', last_update_by=" + Global.myProj.user_id + ", last_update_date='" + dateStr +
   ...
   SET prnt_chck_in_id=" + chckInID +
            ", prnt_doc_typ='Check-In', last_update_by=" + Global.myProj.user_id + ", last_update_date='" + dateStr +
  ...
  string apprvlSts, string nxtApprvl, double invcAmnt)
    {
      ...
      string updtSQL = "UPDATE accb.accb_rcvbls_invc_hdr SET " +
            ...
            "', invoice_amount=" + invcAmnt + ", last_update_by=" + Global.myProj.user_id +
            ", last_update_date='" + dateStr +
...
HAVING b.dbt_amount <> COALESCE(round(SUM(a.dbt_amount),2),0) or COALESCE(round(SUM(a.crdt_amount),2),0) <>  b.crdt_amount
    ...
    public static int getUserStoreID()
    {
      string strSql = "select y.subinv_id " +
        ...
        "y.allow_sales = '1' and z.user_id = " + Global.myProj.user_id +
   ...
   System.Globalization.CultureInfo.InvariantCulture).ToString("yyyy-MM-dd HH:mm:ss");
      string insSQL = "INSERT INTO accb.accb_trnsctn_details(" +
            ...
            func_cur_exchng_rate, accnt_cur_exchng_rate, dbt_or_crdt) " +
               ...
               ", '" + trnsdte.Replace("'", "''") + "', " + crncyid + ", " + Global.myProj.user_id +
               ", '" + dateStr + "', " + batchid + ", " + crdtamnt + ", " +
               Global.myProj.user_id + ", '" + dateStr + "', " + netamnt +
               ", '0', '" + srcids + "', " + entrdAmt +
  ...
  string trnsDate, string src_trsID)
       ...
       long srcDocHdrID, bool autoCalc)
    {
      ...
      string insSQL = "INSERT INTO scm.scm_doc_amnt_smmrys(" +
            ...
            "', " + srcDocHdrID + ", " + Global.myProj.user_id + ", '" + dateStr +
            "', " + Global.myProj.user_id + ", '" + dateStr + "', '" +
            Global.mnFrm.cmCde.cnvrtBoolToBitStr(autoCalc) + "')";
      Global.mnFrm.cmCde.insertDataNoParams(insSQL);
    }
   ...
   System.Globalization.CultureInfo.InvariantCulture).ToString("yyyy-MM-dd");
      ...
      string insSQL = "INSERT INTO scm.scm_sales_invc_hdr(" +
            ...
            "', '" + apprvlSts.Replace("'", "''") + "', " + Global.myProj.user_id + ", '" + dateStr +
            "', " + Global.myProj.user_id + ", '" + dateStr +
            "', '" + nxtApprvl.Replace("'", "''") +
            "', '" + docNum.Replace("'", "''") + "', '" +
      ...
      bool isDlvrd, long otherMdlID, string otherMdlType, string extrDesc)
    {
      ...
      string insSQL = "INSERT INTO scm.scm_sales_invc_det(invc_det_ln_id, " +
            ...
            "," + docID +
            ", " + itmID +
            ", " + qty + ", " + untPrice + ", " + Global.myProj.user_id + ", '" + dateStr +
            "', " + Global.myProj.user_id + ", '" + dateStr +
            "', " + storeID +
   ...
   System.Globalization.CultureInfo.InvariantCulture).ToString("yyyy-MM-dd HH:mm:ss");
      ...
      string insSQL = "INSERT INTO inv.inv_stock_daily_bals(" +
        ...
        "VALUES (" + skckId +
        ", " + totQty + ", " + rsvdQty + ", " + avlblQty + ", '" + balsDate + "', " + Global.myProj.user_id + ", '" + dateStr +
                        "', " + Global.myProj.user_id + ", '" + dateStr + "', ',')";
      Global.mnFrm.cmCde.insertDataNoParams(insSQL);
    }
   ...
   System.Globalization.CultureInfo.InvariantCulture).ToString("yyyy-MM-dd HH:mm:ss");
      string insSQL = "INSERT INTO scm.scm_payments(" +
       ...
       "', " + srcDocID + ", " + Global.myProj.user_id + ", '" + dateStr + "', " +
               Global.myProj.user_id + ", '" + dateStr + "', '" + dateRcvd + "')";
      Global.mnFrm.cmCde.insertDataNoParams(insSQL);
    }
    ...
    string batchdesc, string batchsource)
    {
      ...
      string insSQL = "INSERT INTO accb.accb_trnsctn_batches(" +
       ...
       "', " + Global.myProj.user_id + ", '" + dateStr + "', " + orgid + ", '0', " +
               Global.myProj.user_id + ", '" + dateStr + "', '" +
  ...
  double dbtAmnt, string trnsDate, int crncyid,
   ...
   System.Globalization.CultureInfo.InvariantCulture).ToString("yyyy-MM-dd HH:mm:ss");
      ...
      string insSQL = "INSERT INTO accb.accb_trnsctn_details(" +
            ...
            func_cur_exchng_rate, accnt_cur_exchng_rate, dbt_or_crdt) " +
                        "VALUES (" + accntid + ", '" + trnsDesc.Replace("'", "''") + "', " + dbtAmnt +
                        ", '" + trnsDate + "', " + crncyid + ", " + Global.myProj.user_id + ", '" + dateStr +
                        "', " + batchid + ", " + crdtamnt + ", " + Global.myProj.user_id +
     ...
     string batchdesc, string btchsrc, string batchvldty, long srcbatchid, string avlblforPpstng)
    {
      ...
      "batch_source, batch_vldty_status, src_batch_id, avlbl_for_postng) " +
                        ...
                        "', " + Global.myProj.user_id + ", '" + dateStr +
                        "', " + orgid + ", '0', " + Global.myProj.user_id + ", '" + dateStr +
                        "', '" + btchsrc.Replace("'", "''") +
                        "', '" + batchvldty.Replace("'", "''") +
                        "', " + srcbatchid +
                        ",'" + avlblforPpstng + "')";
   ...
   System.Globalization.CultureInfo.InvariantCulture).ToString("yyyy-MM-dd HH:mm:ss");
    ...
    "SET last_update_by = " + Global.myProj.user_id +
    ", last_update_date = '" + dateStr +
   ...
   System.Globalization.CultureInfo.InvariantCulture).ToString("yyyy-MM-dd HH:mm:ss");
      ...
      string apprvlSts, string nxtApprvl)
    {
      ...
      string updtSQL = "UPDATE scm.scm_sales_invc_hdr SET " +
            "approval_status='" + apprvlSts + "', last_update_by=" + Global.myProj.user_id +
            ", last_update_date='" + dateStr +
            "', next_aproval_action='" + nxtApprvl.Replace("'", "''") +
            "' WHERE (invc_hdr_id = " +
            docid + ")";
      ...
      string apprvlSts)
    {
      ...
      string updtSQL = "UPDATE hotl.checkins_hdr SET " +
            "doc_status='" + apprvlSts + "', last_update_by=" + Global.myProj.user_id +
            ", last_update_date='" + dateStr +
            "' WHERE (check_in_id = " +
...
other_mdls_doc_type = '" + otherMdlType.Replace("'", "''") +
   ...
   System.Globalization.CultureInfo.InvariantCulture).ToString("yyyy-MM-dd");
      ...
      string updtSQL = "UPDATE scm.scm_sales_invc_hdr SET " +
            ...
            "', approval_status='" + apprvlSts.Replace("'", "''") + "', last_update_by=" + Global.myProj.user_id +
            ", last_update_date='" + dateStr +
            "', next_aproval_action='" + nxtApprvl.Replace("'", "''") +
...
GROUP BY customer_id,invc_curr_id";
    ...
    {
      string[] sysLovs = { "Project Types", "Project Size Classifications", "Project Cost Categories", 
                           "Team Classifications","Task Classifications/Objectives",
                           "Resource Classifications", "Project Industry Classifications" };
      string[] sysLovsDesc = { "Project Types","Project Size Classifications", "Project Cost Categories", 
                           "Team Classifications","Task Classifications/Objectives",
                           "Resource Classifications", "Project Industry Classifications"};
      string[] sysLovsDynQrys = { "", "", "", "", "", "", "" };
      string[] pssblVals = { 
        "0", "Design of Plans", "Design of Plans",
        "0", "Design and Build", "Design and Build",
        "0", "Construction", "Construction",
        "0", "Consulting", "Consulting",
        "0", "Administrative", "Administrative",
        "0", "Computer Software Development", "Computer Software Development",
        "0", "Equipment or System Installation", "Equipment or System Installation",
        "0", "Event", "Event",
        "0", "Maintenance", "Maintenance",
        "0", "New Product Development","New Product Development",
        "0", "Research","Research",
        "1", "Large Scale","Large Scale",
        "1", "Medium Scale","Medium Scale",
        "1", "Small Scale","Small Scale",
        "6", "Telecommunications Industry","Telecommunications Industry",
        "6", "Information Technology Industry","Information Technology Industry",
        "6", "Mining Industry","Mining Industry",
        "2", "1. Project Fee/Charge","1Cash Inflow",
        "2", "2. Facility Hiring","2Cash Outflow",
        "2", "3. Equipment Hiring","2Cash Outflow",
        "2", "4. Equipment Purchases","2Cash Outflow",
        "2", "5. Project Administrative Materials","2Cash Outflow",
        "2", "6. Advertisement","2Cash Outflow",
        "2", "7. Catering","2Cash Outflow",
        "2", "8. Resource Persons","2Cash Outflow",
        "2", "9. Administrative Expenses","2Cash Outflow",
        "2", "10. Transportation/Mobilization Expenses","2Cash Outflow",
        "3", "Existing Person","Existing Person",
        "3", "Business Partner","Business Partner",
        "3", "Divisions/Groups","Divisions/Groups",
        "3", "Adhoc Team Member","Adhoc Team Member",        
        "4", "Pre-Execution Tasks","Pre-Execution Tasks",
        "4", "Project Execution Tasks","Project Execution Tasks",
        "4", "Post-Execution Tasks","Post-Execution Tasks",
        "5", "Vehicle","Vehicle",
        "5", "Construction Equipment","Construction Equipment",
        "5", "Earth-Moving Equipment","Earth-Moving Equipment"};
  ...
  =" + pntval + @"  WHERE attnd_rec_id = " + recLineID;
    ...
    public static void refreshRqrdVrbls()
    {
      ...
      Global.mnFrm.cmCde.ModuleAdtTbl = Global.myProj.full_audit_trail_tbl_name;
      Global.mnFrm.cmCde.ModuleDesc = Global.myProj.mdl_description;
      Global.mnFrm.cmCde.ModuleName = Global.myProj.name;
      //Global.mnFrm.cmCde.pgSqlConn = Global.myRpt.Host.globalSQLConn;
      //Global.mnFrm.cmCde.Role_Set_IDs = Global.myRpt.role_set_id;
      Global.mnFrm.cmCde.SampleRole = "Project Management Administrator";
      //Global.mnFrm.cmCde.User_id = Global.myRpt.user_id;
      //Global.mnFrm.cmCde.Org_id = Global.myRpt.org_id;
      Global.mnFrm.cmCde.Extra_Adt_Trl_Info = "";
      Global.myProj.user_id = Global.mnFrm.usr_id;
      Global.myProj.login_number = Global.mnFrm.lgn_num;
      Global.myProj.role_set_id = Global.mnFrm.role_st_id;
      Global.myProj.org_id = Global.mnFrm.Og_id;
